...
async function runSetup() {
  ...
  fs.readFile("./config/site.config.js", "utf8", (err, data) => {
    ...
    fs.writeFile(
      path.join(ROOT, "/config/site.config.js"),
      data,
      "utf8",
      (err) => {}
    );
  ...
  //   fs.writeFile(
  //     path.join(ROOT, "/src/stylesheets/styles.scss"),
  //     cssContent,
  //     (err) => {}
  //   );
  // }
  ...
  if (questions.jquery == "Yes") {
    ...
    fs.writeFile(
      path.join(ROOT, "/src/javascripts/scripts.js"),
      jsContent,
      (err) => {}
    );
  }
